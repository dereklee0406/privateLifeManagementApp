...
/**
 * Purpose: Consistency & Streaks segment — 90-day heatmap, rhythm summary, habit cards.
 * Inputs: ReminderProvider (habits + complete/uncomplete); Settings (reminder types).
 * Outputs: presentation only; streak/matrix math stays in habitStreaks domain.
 * Side effects: navigation to new reminder; completeReminder / uncompleteReminder; success haptic.
 * Design decisions: View filters enabled recurring habits; density fills use theme tokens so
 *   the domain stays framework-free. Consistency rates are 0–1 fractions shown as %.
 */
export function RhythmStreaksPanel() {
  const { t } = useI18n();
   ...
   * Inputs: habit row; whether already completed today.
   * Outputs: none.
